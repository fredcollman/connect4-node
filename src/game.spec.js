...
describe("move", () => {
  ...
  it("can end in a draw", () => {
    const result = newState()
      ...
      .flatMap(move(7))
      .flatMap(move(7));
    expect(result).to.deep.equal(Either.left("Game ended in a draw"));
  });

  it("can be a win for YELLOW on the last move", () => {
    const result = newState()
      .flatMap(move(1))
      .flatMap(move(2))
      .flatMap(move(1))
      ...
      .flatMap(move(6))
      .flatMap(move(4))
      // plus one final move to trigger error
      .flatMap(move(7));
    expect(result).to.deep.equal(Either.left("YELLOW has already won"));
  });
});
